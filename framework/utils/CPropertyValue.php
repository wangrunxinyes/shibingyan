...
class CPropertyValue {
	 ...
	 * If the value is a string and it is
	 ...
	 *
	 * @param string $value
	 *        	the enumerable value to be checked.
	 * @param string $enumType
	 *        	the enumerable class name (make sure it is included before calling this function).
	...
	public static function ensureEnum($value, $enumType) {
		...
		if ($types [$enumType]->hasConstant ( $value ))
			...
			throw new CException ( Yii::t ( 'yii', 'Invalid enumerable value "{value}". Please make sure it is among ({enum}).', array (
					'{value}' => $value,
					'{enum}' => implode ( ', ', $types [$enumType]->getConstants () ) 
			) ) );
	}
}
