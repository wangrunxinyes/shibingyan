...
class CTimestamp {
	 ...
	 * 
	 * @param integer|boolean $d
	 *        	original date timestamp. False to use the current timestamp.
	 * @param boolean $fast
	 *        	false to compute the day of the week, default is true
	 * @param boolean $gmt
	 *        	true to calculate the GMT dates
	 ...
	 *        	second
	 * @param boolean $hs24
	 *        	whether the hours should be 0 through 23 (default) or 1 through 12.
	 * @return boolean true if valid date, semantic check only.
	 */
	 ...
	 * @param integer|boolean $d
	 *        	timestamp
	 * @param boolean $is_gmt
	 *        	whether this is a GMT timestamp
		...
		for($i = 0; $i < $max; $i ++) {
			switch ($fmt [$i]) {
				...
				case 'r' : // Thu, 21 Dec 2000 16:01:07 +0200
					...
					$dates .= gmdate ( 'D', $_day_power * (3 + self::getDayOfWeek ( $year, $month, $day )) ) . ', ' . ($day < 10 ? '0' . $day : $day) . ' ' . date ( 'M', mktime ( 0, 0, 0, $month, 2, 1971 ) ) . ' ' . $year . ' ';
	 ...
	 *        	year
	 * @param boolean $is_gmt
	 *        	whether this is GMT time. If true, gmmktime() will be used.
