...
abstract class CConsoleCommand extends CComponent {
	...
	public function run($args) {
		...
		foreach ( $method->getParameters () as $i => $param ) {
			...
			if (isset ( $options [$name] )) {
				if ($param->isArray ())
					$params [] = is_array ( $options [$name] ) ? $options [$name] : array (
							$options [$name] 
					);
	 ...
	 *        	the application exit code returned by the action method.
			...
			if (preg_match ( '/^--(\w+)(=(.*))?$/', $arg, $matches )) // an option
{
	 ...
	 *        	the default string to be returned when user does not write anything.
	 ...
	 *        	this value is returned if no selection is made. This parameter has been available since version 1.1.11.
