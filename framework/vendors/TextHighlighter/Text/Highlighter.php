	...
	function _getToken() {
		...
		foreach ( $this->_counts [$this->_state] as $i => $count ) {
			...
			if ($m [$n] [1] > - 1 && ($endpos == - 1 || $m [$n] [1] < $endpos)) {
				if ($this->_states [$this->_state] [$i] != - 1) {
					...
					if (isset ( $this->_parts [$this->_state] [$i] )) {
							...
							if (isset ( $this->_parts [$this->_state] [$i] [$j] )) {
								if ($m [$j + $n] [1] > $partpos) {
									array_unshift ( $parts, array (
											$inner,
											substr ( $this->_str, $partpos, $m [$j + $n] [1] - $partpos ) 
									) );
								}
								array_unshift ( $parts, array (
										$this->_parts [$this->_state] [$i] [$j],
										$m [$j + $n] [0] 
								) );
							}
						...
						if ($partpos < $m [$n] [1] + strlen ( $m [$n] [0] )) {
							array_unshift ( $parts, array (
									$inner,
									substr ( $this->_str, $partpos, $m [$n] [1] - $partpos + strlen ( $m [$n] [0] ) ) 
							) );
						}
				...
				if ($m [$n] [1] > $this->_pos) {
					$this->_tokenStack [] = array (
							$this->_lastinner,
							substr ( $this->_str, $this->_pos, $m [$n] [1] - $this->_pos ) 
					);
				}
		...
		if ($endpos > - 1) {
			...
			if ($endpos > $this->_pos) {
				$this->_tokenStack [] = array (
						$this->_lastinner,
						substr ( $this->_str, $this->_pos, $endpos - $this->_pos ) 
				);
			}
