...
class Text_Highlighter_Generator extends XML_Parser {
	...
	function _formatError($code, $params, $fileName, $lineNo) {
		...
		$ret = call_user_func_array ( 'sprintf', array_merge ( array (
				$template 
		), $params ) );
		if ($fileName) {
			$ret = '[' . $fileName . '] ' . $ret;
	...
	/**
	 * Saves generated class to file.
	 * Note that {@link Text_Highlighter::factory()}
	 * assumes that filename is uppercase (SQL.php, DTD.php, etc), and file
	 * is located in Text/Highlighter
	 *
	 * @param string $filename
	 *        	Name of file to write the code to
	...
	function saveCode($filename) {
		$f = @fopen ( $filename, 'wb' );
		if (! $f) {
			$this->_error ( TEXT_HIGHLIGHTER_FILE_WRITE, array (
					'outfile' => $filename 
			) );
			return false;
		}
	...
	function xmltag_Region($xp, $elem, $attribs) {
		...
		$this->_element ['delimGroup'] = isset ( $attribs ['delimGroup'] ) ? $attribs ['delimGroup'] : $attribs ['innerGroup'];
	 ...
	 *        	Error code
	 * @param mixed $message
	 *        	Error message or array with error message parameters
	 * @param integer $lineNo
	...
	function xmltag_Highlight_($xp, $elem) {
		...
		foreach ( $this->_keywords as $name => $keyword ) {
			if (isset ( $keyword ['ifdef'] )) {
				$conditions [$keyword ['ifdef']] [] = array (
						$name,
						true 
				);
			}
			if (isset ( $keyword ['ifndef'] )) {
				$conditions [$keyword ['ifndef']] [] = array (
						$name,
						false 
				);
			}
		...
		foreach ( $syntax ['blocks'] as $name => $block ) {
			...
			if (is_array ( @$syntax ['blocks'] [$name] ['lookfor'] )) {
				usort ( $syntax ['blocks'] [$name] ['lookfor'], array (
						&$this,
						'_sortLookFor' 
				) );
			}
		}
		usort ( $syntax ['toplevel'], array (
				&$this,
				'_sortLookFor' 
		) );
