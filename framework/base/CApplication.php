...
abstract class CApplication extends CModule {
	 ...
	 * @param boolean $exit
	 *        	whether to exit the current request. This parameter has been available since version 1.1.5.
	...
	public function setBasePath($path) {
		if (($this->_basePath = realpath ( $path )) === false || ! is_dir ( $this->_basePath ))
			throw new CException ( Yii::t ( 'yii', 'Application base path "{path}" is not a valid directory.', array (
					'{path}' => $path 
			) ) );
	}
	...
	public function setRuntimePath($path) {
		if (($runtimePath = realpath ( $path )) === false || ! is_dir ( $runtimePath ) || ! is_writable ( $runtimePath ))
			throw new CException ( Yii::t ( 'yii', 'Application runtime path "{path}" is not valid. Please make sure it is a directory writable by the Web server process.', array (
					'{path}' => $path 
			) ) );
	...
	public function setExtensionPath($path) {
		if (($extensionPath = realpath ( $path )) === false || ! is_dir ( $extensionPath ))
			throw new CException ( Yii::t ( 'yii', 'Extension path "{path}" does not exist.', array (
					'{path}' => $path 
			) ) );
	 ...
	 *        	the language that the original file is in. If null, the application {@link sourceLanguage source language} is used.
	 * @param string $language
	 *        	the desired language that the file should be localized to. If null, the {@link getLanguage application language} will be used.
	 ...
	 *        	the locale ID (e.g. en_US). If null, the {@link getLanguage application language ID} will be used.
	...
	public function setGlobalState($key, $value, $defaultValue = null) {
		...
		if ($this->_stateChanged !== $changed)
			$this->attachEventHandler ( 'onEndRequest', array (
					$this,
					'saveGlobalState' 
			) );
	}
	...
	public function loadGlobalState() {
		...
		$this->detachEventHandler ( 'onEndRequest', array (
				$this,
				'saveGlobalState' 
		) );
	}
