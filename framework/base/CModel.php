...
abstract class CModel extends CComponent implements IteratorAggregate, ArrayAccess {
	 ...
	 *        	whether to call {@link clearErrors} before performing validation
	...
	public function createValidators() {
		...
		foreach ( $this->rules () as $rule ) {
			if (isset ( $rule [0], $rule [1] )) // attributes, validator name
				...
				throw new CException ( Yii::t ( 'yii', '{class} has an invalid validation rule. The rule must specify attributes to be validated and the validator name.', array (
	...
	public function onUnsafeAttribute($name, $value) {
		if (YII_DEBUG)
			Yii::log ( Yii::t ( 'yii', 'Failed to set unsafe attribute "{attribute}" of "{class}".', array (
					'{attribute}' => $name,
					'{class}' => get_class ( $this ) 
