...
class CComponent {
	...
	public function __get($name) {
		...
		throw new CException ( Yii::t ( 'yii', 'Property "{class}.{property}" is not defined.', array (
	...
	public function __set($name, $value) {
		...
		if (method_exists ( $this, 'get' . $name ))
			throw new CException ( Yii::t ( 'yii', 'Property "{class}.{property}" is read only.', array (
					'{class}' => get_class ( $this ),
					'{property}' => $name 
			) ) );
		else
	...
	public function __call($name, $parameters) {
		...
		throw new CException ( Yii::t ( 'yii', '{class} and its behaviors do not have a method or closure named "{name}".', array (
				'{class}' => get_class ( $this ),
				'{name}' => $name 
		) ) );
	}
	...
	public function getEventHandlers($name) {
		if ($this->hasEvent ( $name )) {
			...
			throw new CException ( Yii::t ( 'yii', 'Event "{class}.{event}" is not defined.', array (
					'{class}' => get_class ( $this ),
					'{event}' => $name 
			) ) );
	}
				...
				elseif (is_callable ( $handler, true )) {
					if (is_array ( $handler )) {
						...
						elseif (method_exists ( $object, $method ))
							...
							throw new CException ( Yii::t ( 'yii', 'Event "{class}.{event}" is attached with an invalid handler "{handler}".', array (
									'{class}' => get_class ( $this ),
									'{event}' => $name,
									'{handler}' => $handler [1] 
							) ) );
	 ...
	 *
	 * @param mixed $_expression_
	 *        	a PHP expression or PHP callback to be evaluated.
	 * @param array $_data_
