...
class CSecurityManager extends CApplicationComponent {
			...
			if (($key = Yii::app ()->getGlobalState ( self::STATE_VALIDATION_KEY )) !== null)
				...
				if (($key = $this->generateRandomString ( 32, true )) === false)
					if (($key = $this->generateRandomString ( 32, false )) === false)
						throw new CException ( Yii::t ( 'yii', 'CSecurityManager::generateRandomString() cannot generate random string in the current environment.' ) );
				$this->setValidationKey ( $key );
	 ...
	 *        	data to be hashed.
	 * @param string $key
	 *        	the private key to be used for generating HMAC. Defaults to null, meaning using {@link validationKey}.
	 ...
	 * This method has been made public
	 * since 1.1.14.
	 * 
	 * @param string $data
	 *        	data to be generated HMAC.
	 * @param string|null $key
	 *        	the private key to be used for generating HMAC. Defaults to null, meaning using
	 ...
	 * Generates only [0-9a-zA-z_~] characters which are all
	 * transparent in raw URL encoding.
	 * 
	 * @param integer $length
	 *        	length of the generated string in characters.
	 * @param boolean $cryptographicallyStrong
	...
	public function generateRandomString($length, $cryptographicallyStrong = true) {
		if (($randomBytes = $this->generateRandomBytes ( $length + 2, $cryptographicallyStrong )) !== false)
			return strtr ( $this->substr ( base64_encode ( $randomBytes ), 0, $length ), array (
					'+' => '_',
					'/' => '~' 
			) );
		return false;
	 ...
	 * @param boolean $cryptographicallyStrong
	 *        	whether to fail if a cryptographically strong
	...
	public function generateRandomBytes($length, $cryptographicallyStrong = true) {
		...
		if (function_exists ( 'mcrypt_create_iv' ) && ($bytes = mcrypt_create_iv ( $length, MCRYPT_DEV_URANDOM )) !== false && $this->strlen ( $bytes ) >= $length) {
			return $this->substr ( $bytes, 0, $length );
		}
		
		if (($file = @fopen ( '/dev/urandom', 'rb' )) !== false && ($bytes = @fread ( $file, $length )) !== false && (fclose ( $file ) || true) && $this->strlen ( $bytes ) >= $length) {
			return $this->substr ( $bytes, 0, $length );
		}
		
		$i = 0;
		while ( $this->strlen ( $bytes ) < $length && ($byte = $this->generateSessionRandomBlock ()) !== false && ++ $i < 3 ) {
			$bytes .= $byte;
		}
	 ...
	 * On some systems this may be a bit
	...
	public function generatePseudoRandomBlock() {
		...
		if (function_exists ( 'openssl_random_pseudo_bytes' ) && ($bytes = openssl_random_pseudo_bytes ( 512 )) !== false && $this->strlen ( $bytes ) >= 512) {
		...
		if (is_string ( $key )) {
			...
			if ($supportedKeyLengths) {
				if (! in_array ( $this->strlen ( $key ), $supportedKeyLengths )) {
					throw new CException ( Yii::t ( 'yii', 'Encryption key length can be {keyLengths}', array (
							'{keyLengths}' => implode ( ',', $supportedKeyLengths ) . '.' 
					) ) );
				}
				...
				if ($this->strlen ( $key ) < $minLength || $this->strlen ( $key ) > $maxLength)
					throw new CException ( Yii::t ( 'yii', 'Encryption key length must be between {minLength} and {maxLength}.', array (
							'{minLength}' => $minLength,
							'{maxLength}' => $maxLength 
					) ) );
			} else
	 ...
	 *        	data to be decrypted.
	 * @param string $key
	 *        	the decryption key. This defaults to null, meaning the key should be loaded from persistent storage.
	 * @param string|array $cipher
