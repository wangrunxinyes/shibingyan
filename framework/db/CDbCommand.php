...
class CDbCommand extends CComponent {
	 ...
	 * @param mixed $query
	 *        	the DB query to be executed. This can be either
	...
	public function prepare() {
		if ($this->_statement == null) {
			try {
				...
				throw new CDbException ( Yii::t ( 'yii', 'CDbCommand failed to prepare the SQL statement: {error}', array (
						'{error}' => $e->getMessage () 
	 ...
	 *        	the driver-specific options (this is available since version 1.1.6)
	...
	public function execute($params = array()) {
		...
		try {
			...
			Yii::log ( Yii::t ( 'yii', 'CDbCommand::execute() failed: {error}. The SQL statement executed was: {sql}.', array (
					'{error}' => $message,
			...
			throw new CDbException ( Yii::t ( 'yii', 'CDbCommand failed to execute the SQL statement: {error}', array (
					'{error}' => $message 
			) ), ( int ) $e->getCode (), $errorInfo );
		}
	}
	...
	public function queryColumn($params = array()) {
		return $this->queryInternal ( 'fetchAll', array (
				PDO::FETCH_COLUMN,
				0 
		), $params );
	}
	...
	private function queryInternal($method, $mode, $params = array()) {
		...
		try {
			...
			$errorInfo = $e instanceof PDOException ? $e->errorInfo : null;
			$message = $e->getMessage ();
			Yii::log ( Yii::t ( 'yii', 'CDbCommand::{method}() failed: {error}. The SQL statement executed was: {sql}.', array (
					'{method}' => $method,
					'{error}' => $message,
					'{sql}' => $this->getText () . $par 
			) ), CLogger::LEVEL_ERROR, 'system.db.CDbCommand' );
	 ...
	 * 
	 * @param array $query
	 *        	the query specification in name-value pairs. The following
	 ...
	 * 
	 * @param mixed $columns
	 *        	the columns to be selected. Defaults to '*', meaning all columns.
	 ...
	 * 
	 * @param mixed $value
	 *        	the data to be selected. Please refer to {@link select()} for details
	 ...
	 * 
	 * @param mixed $columns
	 *        	the columns to be selected. See {@link select} for more details.
	 ...
	 * 
	 * @param mixed $tables
	 *        	the table(s) to be selected from. This can be either a string (e.g. 'tbl_user')
	 ...
	 * 
	 * @param mixed $value
	 *        	the tables to be selected from. Please refer to {@link from()} for details
	 *        	on how to specify this parameter.
	 * @since 1.1.6
	 */
	 ...
	 *
	 * @param mixed $conditions
	 *        	the conditions that should be appended to the WHERE part.
	 * @param array $params
	 *        	the parameters (name=>value) to be bound to the query.
	 * @return static the command object itself.
	 * @since 1.1.13
	...
	public function andWhere($conditions, $params = array()) {
		if (isset ( $this->_query ['where'] ))
			$this->_query ['where'] = $this->processConditions ( array (
					'AND',
					$this->_query ['where'],
					$conditions 
			) );
	 ...
	 * 
	 * @param mixed $value
	 *        	the where part. Please refer to {@link where()} for details
	 *        	on how to specify this parameter.
	 * @since 1.1.6
	 */
	 ...
	 * 
	 * @param mixed $value
	 *        	the join part in the query. This can be either a string or
	 ...
	 * 
	 * @param mixed $value
	 *        	the GROUP BY part. Please refer to {@link group()} for details
	 *        	on how to specify this parameter.
	 * @since 1.1.6
	 */
	 ...
	 * 
	 * @param mixed $value
	 *        	the HAVING part. Please refer to {@link having()} for details
	 *        	on how to specify this parameter.
	 * @since 1.1.6
	 */
	 ...
	 * 
	 * @param mixed $value
	 *        	the ORDER BY part. Please refer to {@link order()} for details
	 *        	on how to specify this parameter.
	 * @since 1.1.6
	 */
	 ...
	 * 
	 * @param integer $value
	 *        	the LIMIT part. Please refer to {@link limit()} for details
	 *        	on how to specify this parameter.
	 * @since 1.1.6
	 */
	 ...
	 * 
	 * @param integer $value
	 *        	the OFFSET part. Please refer to {@link offset()} for details
	 *        	on how to specify this parameter.
	 * @since 1.1.6
	 */
	 ...
	 * 
	 * @param mixed $value
	 *        	the UNION part. This can be either a string or an array
	 ...
	 * @param string $type
	 *        	the column type. The {@link getColumnType} method will be invoked to convert abstract column type (if any)
	 ...
	 *        	the name of the column to that the constraint will be added on. If there are multiple columns, separate them with commas or pass as an array of column names.
	 * @param string $refTable
	 *        	the table that the foreign key references to.
	 * @param string|array $refColumns
	 *        	the name of the column that the foreign key references to. If there are multiple columns, separate them with commas or pass as an array of column names.
	 * @param string $delete
	 ...
	 * 
	 * @param mixed $conditions
	 *        	the conditions that will be put in the WHERE part.
	...
	private function processConditions($conditions) {
		...
		throw new CDbException ( Yii::t ( 'yii', 'Unknown operator "{operator}".', array (
				'{operator}' => $operator 
		) ) );
	}
	 ...
	 * 
	 * @param string $type
	 *        	the join type ('join', 'left join', 'right join', 'cross join', 'natural join')
	 * @param string $table
	 *        	the table to be joined.
	 *        	Table name can contain schema prefix (e.g. 'public.tbl_user') and/or table alias (e.g. 'tbl_user u').
	 *        	The method will automatically quote the table name unless it contains some parenthesis
	 ...
	 * 
	 * @param string $name
	 *        	the name of the primary key constraint to be created. The name will be properly quoted by the method.
	 * @param string $table
	 *        	the table who will be inheriting the primary key. The name will be properly quoted by the method.
	 * @param string|array $columns
	 ...
	 * 
	 * @param string $name
	 *        	the name of the primary key constraint to be dropped. The name will be properly quoted by the method.
	 * @param string $table
	 *        	the table that owns the primary key. The name will be properly quoted by the method.
	 * @return integer number of rows affected by the execution.
	 * @since 1.1.13
	 */
