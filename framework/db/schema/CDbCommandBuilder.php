...
class CDbCommandBuilder extends CComponent {
	 ...
	 * 
	 * @param mixed $table
	 *        	the table schema ({@link CDbTableSchema}) or the table name (string).
	...
	public function createInsertCommand($table, $data) {
		...
		if ($fields === array ()) {
			$pks = is_array ( $table->primaryKey ) ? $table->primaryKey : array (
					$table->primaryKey 
			);
			foreach ( $pks as $pk ) {
	 ...
	 *        	the table schema ({@link CDbTableSchema}) or the table name (string).
	 * @param array[] $data
	 *        	list data to be inserted, each value should be an array in format (column name=>column value).
	...
	public function createUpdateCommand($table, $data, $criteria) {
		...
		if ($fields === array ())
			throw new CDbException ( Yii::t ( 'yii', 'No columns are being updated for table "{table}".', array (
					'{table}' => $table->name 
			) ) );
	...
	public function createUpdateCounterCommand($table, $counters, $criteria) {
		...
		if ($fields !== array ()) {
			...
			throw new CDbException ( Yii::t ( 'yii', 'No counter columns are being updated for table "{table}".', array (
					'{table}' => $table->name 
			) ) );
	}
		...
		} elseif (is_array ( $columnName )) // composite key: $values=array(array('pk1'=>'v1','pk2'=>'v2'),array(...))
{
	 ...
	 * 
	 * @param mixed $table
	 *        	table schema ({@link CDbTableSchema}) or table name (string).
	...
	protected function ensureTable(&$table) {
		if (is_string ( $table ) && ($table = $this->_schema->getTable ( $tableName = $table )) === null)
			throw new CDbException ( Yii::t ( 'yii', 'Table "{table}" does not exist.', array (
					'{table}' => $tableName 
			) ) );
	}
	
	/**
	 * Returns default value of the integer/serial primary key.
	 * Default value means that the next
