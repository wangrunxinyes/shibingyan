...
EOD;
	...
	public function resetSequence($table, $value = null) {
		...
		$this->getDbConnection ()->createCommand ( "CREATE SEQUENCE \"{$table->name}_SEQ\" START WITH {$value} INCREMENT BY 1 NOMAXVALUE NOCACHE" )->execute ();
	}
	...
	public function checkIntegrity($check = true, $schema = '') {
		...
		foreach ( $this->getTableNames ( $schema ) as $table ) {
			$constraints = $this->getDbConnection ()->createCommand ( "SELECT CONSTRAINT_NAME FROM USER_CONSTRAINTS WHERE TABLE_NAME=:t AND OWNER=:o" )->queryColumn ( array (
					':t' => $table,
					':o' => $schema 
			) );
