...
abstract class CActiveRecord extends CModel {
	 ...
	 *        	scenario name. See {@link CModel::scenario} for more details about this parameter.
	 ...
	 * 
	 * @param string $name
	 *        	the relation name (see {@link relations})
	 ...
	 * @param mixed $params
	 *        	array or CDbCriteria object with additional parameters that customize the query conditions as specified in the relation declaration.
		...
		if ($params !== array ()) // dynamic query
{
	 ...
	 *        	including default scope. This parameter available since 1.1.12
	 ...
	 * This method is used internally by {@link CActiveFinder} to populate
	 ...
	 * 
	 * @param CDbCriteria $criteria
	 *        	the query criteria. This parameter may be modified by merging {@link dbCriteria}.
	 */
	 ...
	 *        	whether to call {@link afterFind} after the record is populated.
	 ...
	 * @param boolean $callAfterFind
	 *        	whether to call {@link afterFind} after each record is populated.
	 * @param string $index
	 *        	the name of the attribute whose value will be used as indexes of the query result array.
...
class CActiveRecordMetaData {
	...
	public function __construct($model) {
		...
		if (($table = $model->getDbConnection ()->getSchema ()->getTable ( $tableName )) === null)
			throw new CDbException ( Yii::t ( 'yii', 'The table "{table}" for active record class "{class}" cannot be found in the database.', array (
	...
	public function addRelation($name, $config) {
		if (isset ( $config [0], $config [1], $config [2] )) // relation class, AR class, FK
			...
			throw new CDbException ( Yii::t ( 'yii', 'Active record "{class}" has an invalid configuration for relation "{relation}". It must specify the relation type, the related active record class and the foreign key.', array (
					'{class}' => $this->_modelClassName,
					'{relation}' => $name 
			) ) );
	}
