...
class CActiveFinder extends CComponent {
	 ...
	 * This method is used internally to perform the relational query
	 ...
	 * 
	 * @param CActiveRecord $baseRecord
	 ...
	 * 
	 * @param CJoinElement $parent
	 *        	the parent tree node
	 * @param mixed $with
	 *        	the names of the related objects relative to the parent tree node
	 * @param array $options
	 *        	additional query options to be merged with the relation
		...
		if (is_string ( $with )) {
			...
			if ($relation instanceof CStatRelation)
				...
				if (! empty ( $relation->through )) {
					$slave = $this->buildJoinTree ( $parent, $relation->through, array (
							'select' => '' 
					) );
		...
		} elseif (! $this->_joined && ! empty ( $this->_parent->records )) // not joined before
{
	 ...
	 * 
	 * @param CActiveRecord $baseRecord
	 *        	the active record whose related object is to be fetched.
	 */
	public function lazyFind($baseRecord) {
		...
		$query->selects = array (
				$child->getColumnSelect ( $child->relation->select ) 
		);
		...
		else // has_many and many_many
{
	 ...
	 * 
	 * @param CJoinQuery $query
	 *        	represents a JOIN SQL statements
				...
				if (isset ( $joinTable->foreignKeys [$fk] )) // FK defined
{
	 ...
	 * 
	 * @param mixed $baseRecords
	 *        	the available base record(s).
	 */
	 ...
	 * 
	 * @param CJoinQuery $query
	 *        	the query being built up
	 */
	public function buildQuery($query) {
		foreach ( $this->children as $child ) {
			if ($child->master !== null)
				$child->_joined = true;
			elseif ($child->relation instanceof CHasOneRelation || $child->relation instanceof CBelongsToRelation || $this->_finder->joinAll || $child->relation->together || (! $this->_finder->baseLimited && $child->relation->together === null)) {
				$child->_joined = true;
				$query->join ( $child );
				$child->buildQuery ( $query );
			}
			...
			else // has_many and many_many
{
	 ...
	 * 
	 * @param mixed $select
	 *        	columns to be selected. Defaults to '*', indicating all columns.
				...
				} elseif (preg_match ( '/^(.*?)\s+AS\s+(\w+)$/im', $name, $matches )) // if the column is already aliased
{
					...
					throw new CDbException ( Yii::t ( 'yii', 'Active record "{class}" is trying to select an invalid column "{column}". Note, the column must exist in the table or be an expression with alias.', array (
	 ...
	 * 
	 * @param CJoinElement $fke
	 *        	the join element containing foreign keys
	 * @param array $fks
	 *        	the foreign keys
	 * @param CJoinElement $pke
	 *        	the join element contains primary keys
	 * @param CJoinElement $parent
	 *        	the parent join element
	...
	private function joinOneMany($fke, $fks, $pke, $parent) {
		...
		foreach ( $fks as $i => $fk ) {
			...
			if (! isset ( $fke->_table->columns [$fk] ))
				throw new CDbException ( Yii::t ( 'yii', 'The relation "{relation}" in active record class "{class}" is specified with an invalid foreign key "{key}". There is no such column in the table "{table}".', array (
						'{class}' => get_class ( $parent->model ),
						'{relation}' => $this->relation->name,
						'{key}' => $fk,
				...
				else // FK constraints undefined
{
	 ...
	 * 
	 * @param CDbTableSchema $joinTable
	 *        	the join table
	 * @param array $fks
	 *        	the foreign keys
	 * @param CJoinElement $parent
	...
	private function joinManyMany($joinTable, $fks, $parent) {
		...
		if ($parentCondition !== array () && $childCondition !== array ()) {
			...
			if (is_array ( $this->relation->joinOptions ) && isset ( $this->relation->joinOptions [0] ) && is_string ( $this->relation->joinOptions [0] ))
			...
			if (is_array ( $this->relation->joinOptions ) && isset ( $this->relation->joinOptions [1] ) && is_string ( $this->relation->joinOptions [1] ))
...
class CJoinQuery {
	 ...
	 * Constructor.
	 * 
	 * @param CJoinElement $joinElement
	 *        	The root join tree.
	 * @param CDbCriteria $criteria
	 *        	the query criteria
	 */
	 ...
	 * 
	 * @param CDbCommandBuilder $builder
	 *        	the command builder
...
class CStatElement {
	 ...
	 * @param CActiveFinder $finder
	 *        	the finder
	 * @param CStatRelation $relation
	 *        	the STAT relation
	 * @param CJoinElement $parent
	 *        	the join element owning this STAT element
	 */
	...
	private function queryOneMany() {
		...
		if (count ( $fks ) !== count ( $pkTable->primaryKey ))
			throw new CDbException ( Yii::t ( 'yii', 'The relation "{relation}" in active record class "{class}" is specified with an invalid foreign key. The columns in the key must match the primary keys of the table "{table}".', array (
					'{class}' => get_class ( $parent->model ),
		...
		foreach ( $fks as $i => $fk ) {
			...
			if (isset ( $table->foreignKeys [$fk] )) {
				...
				if ($schema->compareTableNames ( $pkTable->rawName, $tableName ))
					...
					throw new CDbException ( Yii::t ( 'yii', 'The relation "{relation}" in active record class "{class}" is specified with a foreign key "{key}" that does not point to the parent table "{table}".', array (
							'{class}' => get_class ( $parent->model ),
							'{relation}' => $relation->name,
							'{key}' => $fk,
							'{table}' => $pkTable->name 
		...
		if (count ( $fks ) === 1) // single column FK
{
			...
			$sql = "SELECT $col AS $c, {$relation->select} AS $s FROM {$table->rawName} " . $tableAlias . $join . $where . '(' . $builder->createInCondition ( $table, $fks [0], array_keys ( $records ), $tableAlias . '.' ) . ')' . " GROUP BY $col" . $group . $having . $order;
			$command = $builder->getDbConnection ()->createCommand ( $sql );
		...
		} else // composite FK
{
			...
			$sql = 'SELECT ' . implode ( ', ', $cols ) . ", {$relation->select} AS $s FROM {$table->rawName} " . $tableAlias . $join . $where . '(' . $builder->createInCondition ( $table, $fks, $keys, $tableAlias . '.' ) . ')' . ' GROUP BY ' . implode ( ', ', array_keys ( $cols ) ) . $group . $having . $order;
			$command = $builder->getDbConnection ()->createCommand ( $sql );
			if (is_array ( $relation->params ))
				$builder->bindValues ( $command, $relation->params );
			$stats = array ();
	...
	private function queryManyMany($joinTableName, $keys) {
		...
		if (($joinTable = $builder->getSchema ()->getTable ( $joinTableName )) === null)
			throw new CDbException ( Yii::t ( 'yii', 'The relation "{relation}" in active record class "{class}" is not specified correctly: the join table "{joinTable}" given in the foreign key cannot be found in the database.', array (
					'{class}' => get_class ( $this->_parent->model ),
					'{relation}' => $relation->name,
					'{joinTable}' => $joinTableName 
			) ) );
