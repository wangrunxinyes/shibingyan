...
class CHttpSession extends CApplicationComponent implements IteratorAggregate, ArrayAccess, Countable {
	...
	public function setSavePath($value) {
		if (is_dir ( $value ))
			...
			throw new CException ( Yii::t ( 'yii', 'CHttpSession.savePath "{path}" is not a valid directory.', array (
					'{path}' => $value 
			) ) );
	}
	 ...
	 * 
	 * @param array $value
	 *        	cookie parameters, valid keys include: lifetime, path,
	 ...
	 *
	 * @param string $value
	 *        	how to use cookie to store session ID. Valid values include 'none', 'allow' and 'only'.
	 */
	...
	public function setGCProbability($value) {
		if ($value >= 0 && $value <= 100) {
			...
			throw new CException ( Yii::t ( 'yii', 'CHttpSession.gcProbability "{value}" is invalid. It must be a float between 0 and 100.', array (
					'{value}' => $value 
			) ) );
	}
