...
class CWebUser extends CApplicationComponent implements IWebUser {
	 ...
	 *        	number of seconds that the user can remain in logged-in status. Defaults to 0, meaning login till the user closes the browser.
		...
		if ($this->beforeLogin ( $id, $states, false )) {
			...
			if ($duration > 0) {
				if ($this->allowAutoLogin)
					...
					throw new CException ( Yii::t ( 'yii', '{class}.allowAutoLogin must be set true in order to use cookie-based authentication.', array (
	 ...
	 *        	whether to destroy the whole session. Defaults to true. If false,
	...
	/**
	 * Returns the unique identifier for the user (e.g.
	 * username).
	...
	/**
	 * Sets the unique identifier for the user (e.g.
	 * username).
	 * 
	 * @param string $value
	 *        	the user name.
	 ...
	 *        	the default return URL in case it was not set previously. If this is null,
	 ...
	 * 
	 * @param mixed $id
	 *        	the user ID. This is the same as returned by {@link getId()}.
	 * @param array $states
	 *        	a set of name-value pairs that are provided by the user identity.
	 ...
	 *        	variable value
	 * @param mixed $defaultValue
	 *        	default value. If $value===$defaultValue, the variable will be
	 ...
	 *        	whether to delete the flash messages after calling this method.
	 ...
	 * 
	 * @param string $key
	 *        	key identifying the flash message
	 * @param mixed $defaultValue
	 *        	value to be returned if the flash message is not available.
	 * @param boolean $delete
	 *        	whether to delete this flash message after accessing it.
	 *        	Defaults to true.
	 ...
	 *        	flash message
	 * @param mixed $defaultValue
	 *        	if this value is the same as the flash message, the flash message
	 ...
	 * @param array $params
	 *        	name-value pairs that would be passed to business rules associated
	 ...
	 *        	whether to allow caching the result of access check.
	 *        	When this parameter
