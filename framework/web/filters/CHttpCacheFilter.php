...
/**
 * CHttpCacheFilter implements http caching.
 * It works a lot like {@link COutputCache}
...
class CHttpCacheFilter extends CFilter {
	...
	public function preFilter($filterChain) {
		...
		if (! in_array ( Yii::app ()->getRequest ()->getRequestType (), array (
				'GET',
				'HEAD' 
		) ))
			return true;
	...
	protected function getLastModifiedValue() {
		if ($this->lastModifiedExpression) {
			...
			elseif (($lastModified = strtotime ( $value )) === false)
				throw new CException ( Yii::t ( 'yii', 'Invalid expression for CHttpCacheFilter.lastModifiedExpression: The evaluation result "{value}" could not be understood by strtotime()', array (
						'{value}' => $value 
				) ) );
			return $lastModified;
		}
