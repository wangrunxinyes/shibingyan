...
class CUrlManager extends CApplicationComponent {
	 ...
	 * 
	 * @param array $rules
	 *        	new URL rules (pattern=>route).
	 * @param boolean $append
	 *        	whether the new URL rules should be appended to the existing ones. If false,
	 ...
	 * 
	 * @param mixed $route
	 *        	the route part of the rule. This could be a string or an array
	 ...
	 * 
	 * @param string $pathInfo
	 *        	path info part in the URL
	 * @param string $urlSuffix
	 *        	the URL suffix to be removed
...
abstract class CBaseUrlRule extends CComponent {
	 ...
	 *        	the route
	 * @param array $params
	 *        	list of parameters (name=>value) associated with the route
	 * @param string $ampersand
	 *        	the token separating name-value pairs in the URL.
	 ...
	 *        	the request object
	 * @param string $pathInfo
	 *        	path info part of the URL (URL suffix is already removed based on {@link CUrlManager::urlSuffix})
	 * @param string $rawPathInfo
	 *        	path info that contains the potential URL suffix
...
class CUrlRule extends CBaseUrlRule {
	 ...
	 * 
	 * @param string $route
	 *        	the route of the URL (controller/action)
	 * @param string $pattern
	 *        	the pattern for matching the URL
	 */
	public function __construct($route, $pattern) {
		...
		if (YII_DEBUG && @preg_match ( $this->pattern, 'test' ) === false)
			throw new CException ( Yii::t ( 'yii', 'The URL pattern "{pattern}" for route "{route}" is not a valid regular expression.', array (
					'{route}' => $route,
					'{pattern}' => $pattern 
			) ) );
	}
