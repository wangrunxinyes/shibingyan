...
class COutputCache extends CFilterWidget {
	 ...
	 *        	list of filters being applied to an action
	...
	protected function checkContentCache() {
		if ((empty ( $this->requestTypes ) || in_array ( Yii::app ()->getRequest ()->getRequestType (), $this->requestTypes )) && ($this->_cache = $this->getCache ()) !== null) {
	 ...
	 *        	a property name of the controller. The property should refer to an object
	...
	protected function replayActions() {
		...
		foreach ( $this->_actions as $action ) {
			...
			elseif ($action [0] === '' && function_exists ( $action [1] ))
				...
				throw new CException ( Yii::t ( 'yii', 'Unable to replay the action "{object}.{method}". The method does not exist.', array (
						'object' => $action [0],
						'method' => $action [1] 
				) ) );
		}
