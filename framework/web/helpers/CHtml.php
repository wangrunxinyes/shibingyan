...
class CHtml {
	 ...
	 *        	the element attributes. The values will be HTML-encoded using {@link encode()}.
	 ...
	 * @param array $htmlOptions
	 *        	additional HTML attributes (see {@link tag})
	 ...
	 * 
	 * @param mixed $action
	 *        	the form action URL (see {@link normalizeUrl} for details about this parameter.)
	 * @param string $method
	...
	public static function beginForm($action = '', $method = 'post', $htmlOptions = array()) {
		...
		if (! strcasecmp ( $method, 'get' ) && ($pos = strpos ( $url, '?' )) !== false) {
			foreach ( explode ( '&', substr ( $url, $pos + 1 ) ) as $pair ) {
				if (($pos = strpos ( $pair, '=' )) !== false)
					$hiddens [] = self::hiddenField ( urldecode ( substr ( $pair, 0, $pos ) ), urldecode ( substr ( $pair, $pos + 1 ) ), array (
							'id' => false 
					) );
				else
					$hiddens [] = self::hiddenField ( urldecode ( $pair ), '', array (
							'id' => false 
					) );
			}
		}
		...
		if ($request->enableCsrfValidation && ! strcasecmp ( $method, 'post' ))
			$hiddens [] = self::hiddenField ( $request->csrfTokenName, $request->getCsrfToken (), array (
					'id' => false 
			) );
	 ...
	 * @param array $htmlOptions
	 *        	additional HTML options. The options will be applied to
...
EOD;
		...
		if (empty ( $container ))
			...
			return self::tag ( $container, array (
					'id' => $baseID 
	 ...
	 * @param mixed $url
	 *        	the URL for the AJAX request. If empty, it is assumed to be the current URL. See {@link normalizeUrl} for more details.
	 * @param array $ajaxOptions
	 *        	AJAX options (see {@link ajax})
	 * @param array $htmlOptions
	 *        	additional HTML attributes. Besides normal HTML attributes, a few special
	 *        	attributes are also recognized (see {@link clientChange} and {@link tag} for more details.)
	 ...
	 * 
	 * @param array $options
	 *        	AJAX options. The valid options are used in the form of jQuery.ajax([settings])
	 ...
	 *        	the input value
	 * @param array $htmlOptions
	 *        	additional HTML attributes for the HTML tag (see {@link tag}).
	 ...
	 *        	the attribute name
	 * @param array $htmlOptions
	 *        	additional HTML attributes to be rendered in the container tag.
	 ...
	 *
	 * @param array $models
	 *        	a list of model objects. This parameter
	 *        	can also be an array of associative arrays (e.g. results of {@link CDbCommand::queryAll}).
	 * @param mixed $valueField
	 *        	the attribute name or anonymous function (PHP 5.3+) for list option values
	 * @param mixed $textField
	 *        	the attribute name or anonymous function (PHP 5.3+) for list option texts
	 * @param mixed $groupField
	 *        	the attribute name or anonymous function (PHP 5.3+) for list option group names. If empty, no group will be generated.
	...
	/**
	 * Set generator used in the {@link CHtml::modelName()} method.
	 * You can use the `null` value to restore default
	 * generator.
	 *
	 * @param callback|null $converter
	 *        	the new generator, the model or class name will be passed to the this callback
	 ...
	 *        	the option data (see {@link listData})
	 * @param array $htmlOptions
