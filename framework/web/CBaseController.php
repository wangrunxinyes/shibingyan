...
abstract class CBaseController extends CComponent {
	 ...
	 *        	view file path
	 * @param array $data
	 *        	data to be extracted and made available to the view
	 * @param boolean $return
	 *        	whether the rendering result should be returned instead of being echoed
	...
	public function renderFile($viewFile, $data = null, $return = false) {
		...
		if (count ( $this->_widgetStack ) === $widgetCount)
			...
			throw new CException ( Yii::t ( 'yii', '{controller} contains improperly nested widget tags in its view "{view}". A {widget} widget does not have an endWidget() call.', array (
	 ...
	 *        	class name (can be in path alias format)
	 ...
	 * @param array $properties
	 *        	list of initial property values for the widget (Property Name => Property Value)
	 * @param boolean $captureOutput
	 *        	whether to capture the output of the widget. If true, the method will capture
	 ...
	 * 
	 * @param string $id
	 *        	optional tag identifying the method call for debugging purpose.
	...
	public function endWidget($id = '') {
		if (($widget = array_pop ( $this->_widgetStack )) !== null) {
			...
			throw new CException ( Yii::t ( 'yii', '{controller} has an extra endWidget({id}) call in its view.', array (
					'{controller}' => get_class ( $this ),
					'{id}' => $id 
			) ) );
	}
	 ...
	 *        	the clip ID.
	 * @param array $properties
	 *        	initial property values for {@link CClipWidget}.
	 */
	 ...
	 * @param array $properties
	 *        	initial property values for {@link COutputCache}.
	 ...
	 * 
	 * @param mixed $view
	 *        	the name of the view that will be used to decorate the content. The actual view script
	...
	public function beginContent($view = null, $data = array()) {
		$this->beginWidget ( 'CContentDecorator', array (
				'view' => $view,
				'data' => $data 
		) );
	}
