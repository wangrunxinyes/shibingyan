...
class CWsdlGenerator extends CComponent {
	...
	/**
	 *
	 * @var array soap:body operation style options
	 */
	...
	/**
	 *
	 * @var array soap:operation style
	 */
	public $bindingStyle = self::STYLE_RPC;
	/**
	 *
	 * @var string soap:operation transport
	 */
	...
	protected function processMethod($method) {
		...
		if ($this->bindingStyle == self::STYLE_RPC) {
			if (preg_match ( '/^@return\s+([\w\.]+(\[\s*\])?)\s*?(.*)$/im', $comment, $matches ))
				...
				$return = null;
			$this->messages [$methodName . 'Out'] = array (
					'return' => $return 
			);
		} else {
			if (preg_match ( '/^@return\s+([\w\.]+(\[\s*\])?)\s*?(.*)$/im', $comment, $matches )) {
			...
			$this->messages [$methodName . 'Out'] = array (
					'parameters' => array (
							'element' => 'tns:' . $methodName . 'Response' 
					) 
			);
		}
	 ...
	 *        	XML WSDL document being generated
	 * @param DOMElement $target
	 *        	XML node, to which will be appended $source node
	 * @param DOMNode $source
	 *        	Source XML node to be imported
	 */
...
<definitions name=\"{$this->serviceName}\" targetNamespace=\"{$this->namespace}\"
	 ...
	 *        	method name
	 * @param array $headers
	 *        	array like array('input'=>array(MESSAGE,PART),'output=>array(MESSAGE,PART))
	 */
	 ...
	 *
	 * @param bool $return
	 *        	If true, generated HTML output will be returned rather than directly sent to output buffer
	 */
