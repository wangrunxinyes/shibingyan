...
$table->foreignKeys [$pk [0]] [0] !== $table->foreignKeys [$pk [1]] [0]); // and the foreign keys point different tables
	...
	protected function generateClassName($tableName) {
		return str_replace ( ' ', '', ucwords ( trim ( strtolower ( str_replace ( array (
				'-',
				'_' 
	 ...
	 *        	a regular expression that may be used to filter table names
	 */
	 ...
	 * 
	 * @param string $tableName
	 *        	the name of the table to hold the relation
	 * @param string $fkName
	 *        	the foreign key name
	 * @param boolean $multiple
	 *        	whether the relation would contain multiple objects
