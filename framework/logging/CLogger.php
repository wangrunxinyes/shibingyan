...
class CLogger extends CComponent {
	 ...
	 *        	list of log categories to ignore
	...
	public function getLogs($levels = '', $categories = array(), $except = array()) {
		...
		if (! empty ( $levels ))
			$ret = array_values ( array_filter ( $ret, array (
					$this,
					'filterByLevel' 
			) ) );
	 ...
	 *        	element to be filtered
	 * @param integer $index
	 *        	index of the values array to be used for check
	 * @return boolean true if valid timing entry, false if not.
	 */
	...
	public function getProfilingResults($token = null, $categories = null, $refresh = false) {
		...
		if ($categories !== null) {
			$this->_categories = preg_split ( '/[\s,]+/', strtolower ( $categories ), - 1, PREG_SPLIT_NO_EMPTY );
			$timings = array_filter ( $timings, array (
					$this,
					'filterTimingByCategory' 
			) );
		}
		...
		foreach ( $this->_logs as $log ) {
			...
			if (! strncasecmp ( $message, 'begin:', 6 )) {
				...
				if (($last = array_pop ( $stack )) !== null && $last [0] === $token) {
					...
					throw new CException ( Yii::t ( 'yii', 'CProfileLogRoute found a mismatching code block "{token}". Make sure the calls to Yii::beginProfile() and Yii::endProfile() be properly nested.', array (
							'{token}' => $token 
					) ) );
			}
		}
	 ...
	 * 
	 * @param boolean $dumpLogs
	 *        	whether to process the logs immediately as they are passed to log route
	 * @since 1.1.0
	 */
