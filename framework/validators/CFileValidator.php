...
class CFileValidator extends CValidator {
	...
	protected function validateFile($object, $attribute, $file) {
		...
		if ($error == UPLOAD_ERR_INI_SIZE || $error == UPLOAD_ERR_FORM_SIZE || $this->maxSize !== null && $file->getSize () > $this->maxSize) {
			...
			$this->addError ( $object, $attribute, $message, array (
					'{file}' => CHtml::encode ( $file->getName () ),
			...
			elseif ($error == UPLOAD_ERR_PARTIAL)
				throw new CException ( Yii::t ( 'yii', 'The file "{file}" was only partially uploaded.', array (
						'{file}' => CHtml::encode ( $file->getName () ) 
				) ) );
			elseif ($error == UPLOAD_ERR_NO_TMP_DIR)
				throw new CException ( Yii::t ( 'yii', 'Missing the temporary folder to store the uploaded file "{file}".', array (
						'{file}' => CHtml::encode ( $file->getName () ) 
				) ) );
			elseif ($error == UPLOAD_ERR_CANT_WRITE)
				throw new CException ( Yii::t ( 'yii', 'Failed to write the uploaded file "{file}" to disk.', array (
						'{file}' => CHtml::encode ( $file->getName () ) 
				) ) );
			elseif (defined ( 'UPLOAD_ERR_EXTENSION' ) && $error == UPLOAD_ERR_EXTENSION) // available for PHP 5.2.0 or above
				...
				throw new CException ( Yii::t ( 'yii', 'Unable to upload the file "{file}" because of an unrecognized error.', array (
						'{file}' => CHtml::encode ( $file->getName () ) 
				) ) );
		}
