...
class CDataColumn extends CGridColumn {
	...
	public function getFilterCellContent() {
		...
		elseif ($this->filter !== false && $this->grid->filter !== null && $this->name !== null && strpos ( $this->name, '.' ) === false) {
			if (is_array ( $this->filter ))
				return CHtml::activeDropDownList ( $this->grid->filter, $this->name, $this->filter, array (
						'id' => false,
						'prompt' => '' 
				) );
			elseif ($this->filter === null)
				return CHtml::activeTextField ( $this->grid->filter, $this->name, array (
						'id' => false 
				) );
		} else
	...
	public function getHeaderCellContent() {
		if ($this->grid->enableSorting && $this->sortable && $this->name !== null)
			return $this->grid->dataProvider->getSort ()->link ( $this->name, $this->header, array (
					'class' => 'sort-link' 
			) );
