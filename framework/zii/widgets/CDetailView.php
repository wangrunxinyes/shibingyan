...
class CDetailView extends CWidget {
		...
		foreach ( $this->attributes as $attribute ) {
			...
			elseif (isset ( $attribute ['name'] )) {
				if ($this->data instanceof CModel)
					...
					$tr ['{label}'] = ucwords ( trim ( strtolower ( str_replace ( array (
							'-',
							'_',
							'.' 
					), ' ', preg_replace ( '/(?<![A-Z])[A-Z]/', ' \0', $attribute ['name'] ) ) ) ) );
			}
	 ...
	 *
	 * @param array $options
	 *        	config options for this item/attribute from {@link attributes}
	 * @param string $templateData
	 *        	data that will be inserted into {@link itemTemplate}
	 * @since 1.1.11
	 */
