 ...
 * It can be used in forms and post its value.
 *
...
class CJuiSliderInput extends CJuiInputWidget {
	...
	public function run() {
		...
		$isRange = isset ( $this->options ['range'] ) && $this->options ['range'] && $this->options ['range'] !== 'max' && $this->options ['range'] !== 'min';
		...
		$this->options [$this->event] = $isRange ? new CJavaScriptExpression ( "function(e,ui){ v=ui.values; jQuery('#{$idHidden}').val(v[0]); jQuery('#{$idHidden}{$this->maxIdSuffix}').val(v[1]); }" ) : new CJavaScriptExpression ( "function(event, ui) { jQuery('#{$idHidden}').val(ui.value); }" );
